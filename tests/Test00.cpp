...
TEST_CASE("WorkerPool per_threads_indices_subdivision", "[misc]") {
  SECTION("No rest, a single thread") {
    auto values = per_threads_indices_subdivision(1, 10);
    std::vector<std::pair<std::size_t, std::size_t>> expected{{0, 10}};
    CHECK(values == expected);
  }

  SECTION("More threads than elements") {
    auto values = per_threads_indices_subdivision(4, 2);
    std::vector<std::pair<std::size_t, std::size_t>> expected{
        {0, 1}, {1, 2}, {2, 2}, {2, 2}};
    CHECK(values == expected);
  }

  SECTION("No rest, mutliple threads") {
    auto values = per_threads_indices_subdivision(4, 12);
    std::vector<std::pair<std::size_t, std::size_t>> expected{
        {0, 3}, {3, 6}, {6, 9}, {9, 12}};
    CHECK(values == expected);
  }

  SECTION("Some rest, mutliple threads") {
    auto values = per_threads_indices_subdivision(4, 14);
    std::vector<std::pair<std::size_t, std::size_t>> expected{
        {0, 4}, {4, 8}, {8, 11}, {11, 14}};
    CHECK(values == expected);
  }
