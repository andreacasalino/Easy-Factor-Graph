...
// void compare(const std::vector<float>& probTheory, const std::vector<float>&
// probComputed, const float threshold = 0.01f) {
...
//                 for (auto m = (*c)->activeConnections.begin(); m !=
//                 (*c)->activeConnections.end(); ++m) {
//                     EXPECT_TRUE(nullptr != m->second.message2This);
//                 }
//             }
//         };
//         for (auto it = this->hidden.clusters.begin(); it !=
//         this->hidden.clusters.end(); ++it) {
//             checkCluster(*it);
//         }
//     };
// };
...
//   this->getMarginalDistribution("A")); this->checkMessages(); compare({ (g +
//   e) ,(1 + g * e) }, this->getMarginalDistribution("B")); compare({ (b * (g +
//   e) + (1 + g * e)), ((g + e) + b * (1 + g * e)) },
//   this->getMarginalDistribution("C")); compare({ 1.f, e },
//   this->getMarginalDistribution("D"));
...
//   this->getMarginalDistribution("D"), 0.045f); this->checkMessages();
//   compare({ M_alfa, M_beta }, this->getMarginalDistribution("C"), 0.045f);
//   compare({ M_alfa, M_beta }, this->getMarginalDistribution("B"), 0.045f);
//   compare({ M * M_alfa + M_beta, M_alfa + M * M_beta },
//   this->getMarginalDistribution("A"), 0.045f);
// }
...
//   std::vector<VariablePtr> Y = {makeVariable(2, "Y0"), makeVariable(2,
//   "Y1"),makeVariable(2, "Y2"),makeVariable(2, "Y3")};
//   std::vector<VariablePtr> X = {makeVariable(2, "X0"), makeVariable(2,
//   "X1"),makeVariable(2, "X2"),makeVariable(2, "X3")};

//   auto connect = [&model](const VariablePtr& a, const VariablePtr& b, const
//   float w){
//     model.insertCopy(factor::cnst::FactorExponential(factor::cnst::Factor(std::set<VariablePtr>{a,
//     b}, true) , w));
//   };
...
//   model.insertCopy(factor::cnst::FactorExponential(factor::cnst::IndicatorFactor(Y[0],
//   1), wYY));
...
//   //build the chain
//   model::Graph graph;
//   graph.insert(std::make_shared<factor::modif::FactorExponential>(factor::cnst::Factor({
//   A, B }, true), alfa));
//   graph.insert(std::make_shared<factor::modif::FactorExponential>(factor::cnst::Factor({
//   B, C }, true), beta));
//   graph.insert(std::make_shared<factor::modif::FactorExponential>(factor::cnst::Factor({
...
//           1.f,
//           expf(alfa),
//           expf(alfa) * expf(beta)} , graph.getJointMarginalDistribution({"A",
//           "B", "C"}).getProbabilities());

//   // joint distribution of A B
//   compare({ expf(alfa), 1.f, 1.f, expf(alfa) },
