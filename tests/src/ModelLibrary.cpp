...
structure::ModelSeed SmallCRF::make() {
  structure::ModelBuilder builder;

  auto make_level_ = [&](std::size_t len, bool ev) {
    std::vector<std::size_t> res;
    for (std::size_t i = 0; i < len; ++i) {
      if (ev) {
        res.push_back(builder.make_variable(
            3, structure::ModelBuilder::VariableDescription{
                   .isPermanentEvidence = true}));
      } else {
  ...
  std::vector<std::size_t> o = make_level_(4, true);
  std::vector<std::size_t> h0 = make_level_(3, false);
  std::vector<std::size_t> h1 = make_level_(2, false);
...
SmallCRF::SmallCRF() : EFG::model::ConditionalRandomField{make()} {}
} // namespace EFG::test
