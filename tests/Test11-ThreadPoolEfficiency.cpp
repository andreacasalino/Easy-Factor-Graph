...
class BinaryStructure : public RandomField {
public:
  BinaryStructure(const std::size_t levels, bool loopy) {
    ...
    for (std::size_t k = 1; k < levels; ++k) {
      for (auto it = toExpand.begin(); it != toExpand.end(); ++it) {
        ...
        this->insertTunable(std::make_shared<factor::modif::FactorExponential>(
            factor::cnst::Factor(std::set<categoric::VariablePtr>{*it, varA},
                                 true),
            1.f));
        this->insertTunable(std::make_shared<factor::modif::FactorExponential>(
            factor::cnst::Factor(std::set<categoric::VariablePtr>{*it, varB},
                                 true),
            1.f));
        if (loopy) {
          this->insertTunable(
              std::make_shared<factor::modif::FactorExponential>(
                  factor::cnst::Factor(
                      std::set<categoric::VariablePtr>{varA, varB}, true),
                  1.f));
        }
  ...
  void profile(const Task &task, const std::size_t trials) {
    ...
    case Task::BeliefProp:
      ...
      break;
    case Task::Gibbs:
      t = [&]() { this->getHiddenSetSamples(100, 20); };
      break;
    case Task::Gradient:
      trainSet = std::make_shared<EFG::train::TrainSet>(
          this->getHiddenSetSamples(500, 20));
      t = [&]() { this->getGradient(trainSet); };
      break;
    default:
      break;
    }
    ...
    // !! IMPORTANT !! on local machines this works, showing the effectiveness
    // of the thread pool. However, for reason, on the CI server the execution
