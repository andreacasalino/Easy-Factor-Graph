...
TEST_CASE("Unary factor merge factors", "[operations]") {
  ...
  SECTION("merge exponential factors") {
    ...
    builder.template mergeFactor<true>(make_unary_exp_factor(a, {0.1f, 0, 0}));
    builder.template mergeFactor<true>(make_unary_exp_factor(b, {0, 0.2f, 0}));
    builder.normalize();
...
TEST_CASE("Message passing", "[operations]") {
  ...
  auto edge = factor::BinaryFactor::fromCompactDomain(
