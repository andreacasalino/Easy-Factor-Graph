...
namespace EFG::test {
    std::vector<categoric::Combination> getCombinations(const categoric::Group& group) {
        std::vector<categoric::Combination> combinations;
        combinations.reserve(group.size());
        categoric::Range range(group.getVariables());
        iterator::forEach(range, [&combinations](const categoric::Range& range){
    ...
    FactorBinaryTest::FactorBinaryTest()
        : FactorTest({categoric::makeVariable(3, "A"), categoric::makeVariable(4, "B")}) {
        this->values->emplace(combinations[0], 1.f);
        this->values->emplace(combinations[2], 2.f);
    ...
    FactorTernaryTest::FactorTernaryTest()
        : FactorTest({categoric::makeVariable(3, "A"), categoric::makeVariable(4, "B"), categoric::makeVariable(2, "C")}) {
        this->values->emplace(combinations[0], 1.f);
        this->values->emplace(combinations[2], 2.f);
        this->values->emplace(combinations[4], 3.f);
