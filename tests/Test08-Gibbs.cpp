...
TEST_CASE("binary factor gibbs sampling", "[gibbs_sampling]") {
  ...
  SECTION("specific variable frequency") {
    ...
    REQUIRE(are_samples_valid(samples.makeIter(),
                              {model.getStructure().nodes[0].var_size,
                               model.getStructure().nodes[1].var_size}));
    CHECK(almost_equal(expf(w) / Z, getFrequency1(samples.makeIter(), 1),
                       0.075f));
  }
}
...
std::vector<std::size_t> get_model_sizes(const Structure &structure) {
  auto rng = structure.nodes | std::views::transform([](const Node &node) {
               return node.var_size;
             });
...
TEST_CASE("polyTree gibbs sampling", "[gibbs_sampling]") {
  ...
  REQUIRE(are_samples_valid(samples.makeIter(),
                            get_model_sizes(model.getStructure())));
