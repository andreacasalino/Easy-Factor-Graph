...
std::vector<CombinationPtr>
import(const std::vector<categoric::Combination> &combinations) {
  std::vector<CombinationPtr> temp;
  temp.reserve(combinations.size());
  std::for_each(combinations.begin(), combinations.end(),
                [&temp](const categoric::Combination &c) {
                  temp.emplace_back(
                      std::make_shared<categoric::Combination>(c));
                });
  return temp;
}
...
std::vector<CombinationPtr> import(const std::string &fileName) {
  ...
  auto convert = [](const std::vector<std::string> &slices) {
  ...
  while (!reader.eof()) {
    ...
    values.push_back(std::make_shared<categoric::Combination>(
        convert(xmlPrs::slice_fragments(line))));
  }
  return values;
};
TrainSet::TrainSet(const std::string &fileName) : TrainSet(import(fileName)) {}

TrainSet TrainSet::getRandomSubSet(const float &percentage) const {
  std::size_t numberOfCombinations = 1;
  numberOfCombinations = std::max(static_cast<std::size_t>(std::floor(
                                      this->combinations.size() * percentage)),
                                  numberOfCombinations);
  numberOfCombinations =
      std::min(this->combinations.size(), numberOfCombinations);
  ...
  for (std::size_t k = 0; k < numberOfCombinations; ++k) {
    randomCombinations.emplace_back(
        this->combinations[rand() % this->combinations.size()]);
  }
