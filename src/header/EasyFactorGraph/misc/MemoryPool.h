...
public:
  ...
  template <typename T>
  [[nodiscard]] std::span<T> claimFromPool(std::size_t size);
...
template <typename T> struct Slot {
  ...
  [[nodiscard]] static Slot makeNonOwning(std::span<T> the_view) noexcept {
    Slot res{0};
    res.block = the_view;
    return res;
  }

  [[nodiscard]] static Slot makeOwning(std::span<const T> the_view) noexcept {
    Slot res{the_view.size()};
    std::copy(the_view.begin(), the_view.end(), res.block.begin());
    return res;
  }
  ...
  auto get() const noexcept { return block; }
...
std::span<T> MemoryPool::cloneIntoPool(std::span<T> slot) {
  ...
  std::copy(slot.begin(), slot.end(), claimed.begin());
  return claimed;
}
