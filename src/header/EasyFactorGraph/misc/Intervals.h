...
public:
  ...
  template <bool Sort>
  static Intervals fromPoints(std::vector<IntervalPoint> collection);

  template <bool Sort>
  static Intervals fromChunks(std::vector<Chunk> collection);

  template <typename G>
  requires IsGenerator<G, IntervalPoint>
  static Intervals fromGen(G gen);
  ...
  [[nodiscard]] IntervalsIter iter() const noexcept {
    return IntervalsIter{delimiters.get()};
  }

  const auto &getDelimiters() const noexcept { return delimiters; }
...
private:
  template <typename T> static Intervals fromVec(std::span<const T> collection);
...
template <bool Sort>
Intervals Intervals::fromPoints(std::vector<IntervalPoint> collection) {
  if constexpr (Sort) {
    std::sort(collection.begin(), collection.end(),
              [](const auto &a, const auto &b) { return a.first < b.first; });
  }
  return fromVec(std::span<const IntervalPoint>{collection});
}

template <bool Sort>
Intervals Intervals::fromChunks(std::vector<Chunk> collection) {
  if constexpr (Sort) {
    std::sort(collection.begin(), collection.end(),
              [](const auto &a, const auto &b) { return a.begin < b.begin; });
  }
  return fromVec(std::span<const Chunk>{collection});
}

template <typename T>
Intervals Intervals::fromVec(std::span<const T> collection) {
  using Gen = GenFromBlock<T>;
  return Intervals::fromGen<Gen>(Gen{collection});
}

template <typename G>
requires IsGenerator<G, Intervals::IntervalPoint>
    Intervals Intervals::fromGen(G gen) {
  ...
  return Intervals::fromGen<Gen>(std::move(gen_map));
}

template <typename G>
requires IsGenerator<G, Chunk> Intervals Intervals::fromGen(G gen) {
