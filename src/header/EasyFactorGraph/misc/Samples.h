...
public:
  ...
  [[nodiscard]] SamplesIter makeIter() const noexcept { return {*this}; }

  std::size_t eachSampleSize() const noexcept { return sample_len_; }
  std::size_t samplesCount() const noexcept { return samples_; }
...
void for_each_sample(const Samples &samples, Pred &&pred) {
  for_each_generated<std::span<const categoric::VarStateSize>>(
      samples.makeIter(), std::forward<Pred>(pred));
}
} // namespace EFG::misc
