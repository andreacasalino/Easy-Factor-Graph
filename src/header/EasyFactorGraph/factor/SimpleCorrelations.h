...
public:
  ...
  [[nodiscard]] std::optional<std::pair<categoric::Combination<N>, float>>
  next() noexcept {
...
public:
  SimplyAntiCorrelatedDomainGen(categoric::VarStateSize var_size)
      : prev_same_{0} {
    categoric::Group<N> g{categoric::make_same<N>(var_size)};
  ...
  [[nodiscard]] std::optional<misc::Chunk> next() noexcept {
