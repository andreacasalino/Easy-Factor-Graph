...
public:
  ...
  static FactorT fromCompactDomain(categoric::Combination<N> sizes,
                                   const std::vector<float> &compact_domain);

  template <bool Sort>
  static FactorT
  fromSparseDomain(categoric::Combination<N> sizes,
                   std::vector<SparseDomainPoint<N>> sparse_domain);

  template <typename G>
  requires misc::IsGenerator<G, SparseDomainPoint<N>>
  static FactorT fromSparseDomainGen(categoric::Combination<N> sizes, G gen);
  ...
  template <bool UseTransformed>
  void getValues(std::vector<float> &recipient) const noexcept;

  const auto &getVariables() const noexcept { return variables_; }
...
public:
  ...
  float getTrsfm(std::size_t index) const noexcept {
    return trsfm(values.get()[index]);
  }

  std::span<const float> getAllValues() const noexcept { return values.get(); }
...
template <std::size_t N, typename Transform>
FactorT<N, Transform> FactorT<N, Transform>::fromCompactDomain(
  ...
  return {sizes, misc::Intervals::fromGen<Gen>(std::move(gen_map))};
}

template <std::size_t N, typename Transform>
template <bool Sort>
FactorT<N, Transform> FactorT<N, Transform>::fromSparseDomain(
  ...
  return {sizes,
          misc::Intervals::fromPoints<Sort>(std::move(sparse_domain_map))};
}
...
FactorT<N, Transform>::fromSparseDomainGen(categoric::Combination<N> sizes,
                                           G gen) {
  categoric::Group<N> vars{sizes};
...
template <std::size_t N, typename Transform>
template <bool UseTransformed>
void FactorT<N, Transform>::getValues(
...
template <bool UseTransformed>
void FactorT<1, Transform>::getValues(
    std::vector<float> &recipient) const noexcept {
  recipient.clear();
