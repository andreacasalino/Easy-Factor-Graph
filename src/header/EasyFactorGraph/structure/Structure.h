...
struct Structure {
  ...
  [[nodiscard]] std::vector<float> make_messages_pool_swap() const {
  ...
  [[nodiscard]] auto get_active_incoming(std::size_t var_index) const noexcept {
  ...
  [[nodiscard]] auto
  get_message_dependencies_rng(const MessageMetaData &msg) const noexcept {
  ...
  auto &getListeners() noexcept { return listeners_; }
