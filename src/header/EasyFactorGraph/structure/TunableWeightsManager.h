...
public:
  ...
  [[nodiscard]] TunableWeightsGradient
  gradient(std::shared_ptr<const misc::Samples> training_set) {
    if (training_set->eachSampleSize() != context_->nodes.size()) {
      throw Error{"Invalid training set samples size"};
    }
  ...
  void getTunableWeights(std::vector<float> &recipient) const noexcept;
...
private:
  ...
  std::span<Tuner> tunersSpan(const std::pair<std::size_t, std::size_t> &view) {
    return {tuners_.begin() + view.first, tuners_.begin() + view.second};
  }

  struct GradientScratches {
    std::vector<std::pair<std::size_t, std::size_t>> indices_views;
    std::vector<float> block;
    std::vector<std::vector<float>> scratches;

    void aggregateScratches(std::vector<float> &recipient) {
      recipient.clear();
      for (const auto &scratch : scratches) {
        recipient.insert(recipient.end(), scratch.begin(), scratch.end());
      }
    }
  };
  GradientScratches gradient_scratches_;
};
