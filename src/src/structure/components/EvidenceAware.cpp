...
//             for (auto itConn = (*itOpen)->activeConnections.begin(); itConn
//             != (*itOpen)->activeConnections.end(); ++itConn) {
...
//         std::for_each(toAdd.begin(), toAdd.end(), [this](const
//         std::set<Node*>& c) {
...
//     std::set<categoric::VariablePtr> EvidenceAware::getHiddenVariables()
//     const {
//         std::set<categoric::VariablePtr> hiddens;
//         std::for_each(this->hidden.clusters.begin(),
//         this->hidden.clusters.end(), [&hiddens](const std::set<Node*>&
//         cluster) {
//             std::for_each(cluster.begin(), cluster.end(), [&hiddens](const
//             Node* n) {
...
//     std::set<categoric::VariablePtr> EvidenceAware::getObservedVariables()
//     const {
