  ...
  for (const auto &cluster : clusters) {
    for (auto *node : cluster) {
      for (auto &[connected_node, connection] : node->activeConnections) {
        connection.message2ThisNode.reset();
      }
    }
  }
}
...
BeliefPropagator::propagateBelief(const PropagationKind &kind) {
  ...
  resetMessages(getHiddenClusters());
...
public:
  ...
  inline bool isComputationPossible() const {
    for (auto it = this->dependencies.begin(); it != this->dependencies.end();
         ++it) {
  ...
  float compute() {
    ...
    if (PropagationKind::Sum == kind) {
      if (1 == toMerge.size()) {
        newMessage = std::make_unique<distribution::factor::cnst::MessageSum>(
            **toMerge.begin(),
      ...
      if (1 == toMerge.size()) {
        newMessage = std::make_unique<distribution::factor::cnst::MessageMAP>(
            **toMerge.begin(),
            std::set<categoric::VariablePtr>{sender->variable});
      } else {
        newMessage = std::make_unique<distribution::factor::cnst::MessageMAP>(
...
#ifdef THREAD_POOL_ENABLED
bool BeliefPropagator::messagePassingThreadPool(const std::set<Node *> &cluster,
                                                const PropagationKind &kind) {
  auto openSet = getOpenSet(cluster);
  bool progressWasMade;
  while (!openSet.empty()) {
