...
public:
  ...
  void compute(std::vector<float> &recipient,
    ...
    auto &contributions = cache_.template getBuffer<0>();
    ...
    auto &group_sizes = cache_.template getBuffer<1>();
...
std::span<float> QueryManager::gatherNodeUnaries_(std::size_t variable_index) {
  ...
  use_factor_if(node.unary_factor, [&](const auto &factor) {
    merger_.template mergeFactor<true>(factor);
  });
...
void QueryManager::getJointMarginalDistributionProbabilities_(
  ...
  if (vars.empty()) {
    throw Error{"No variable specified"};
  }
  if (std::unordered_set<std::size_t>{vars.begin(), vars.end()}.size() !=
      vars.size()) {
    throw Error{"Found variables repetition"};
  }
  if (context_->nodes.size() <= *std::max_element(vars.begin(), vars.end())) {
    throw Error{"Some variables is from an invalid index"};
  }
