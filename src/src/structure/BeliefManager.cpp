...
float MessagePassingHandler::compute_message(Structure &context,
  ...
  use_factor_if(context.nodes[msg.factor_info.sender_index].unary_factor,
                [&](const auto &factor) {
                  this->UnaryFactorsMerger::template mergeFactor<true>(factor);
                });

  if constexpr (EvalDiff) {
    auto &prev_distr_snap = this->cache_.template getBuffer<0>();
  ...
  if constexpr (EvalDiff) {
    return eval_diff(this->cache_.getBuffer<0, false>(), recipient);
  } else {
...
namespace {
bool can_compute_message(Structure &context,
...
void first_propagation(MessagePassingHandler &mp, Structure &context,
  ...
  while (!open.empty()) {
    ...
    to_close.clear();
    for (auto conn : open) {
      if (can_compute_message(context, conn)) {
...
void BeliefManager::SerialBeliefPropagator::propagate(Structure &ctxt,
  ...
  if (cluster.order.has_value()) {
    cluster.order->deterministic_part.forEach(
...
namespace {
void calibrate_before_reset(Structure &ctxt,
                            const std::vector<std::size_t> &to_calibrate) {
  for (auto conn_idx : to_calibrate) {
...
BeliefManager::SerialBeliefPropagator::calibration_(
  ...
  calibrate_before_reset(ctxt, to_calibrate_part);
...
void BeliefManager::ConcurrentBeliefPropagator::propagate(
  ...
  if (cluster.order.has_value()) {
    cluster.order->deterministic_part.forEachLevel([&](auto level) {
