...
struct ConversionBinary {
  static void from_json(const nlohmann::json &giver, ModelBuilder &model) {
    ...
    if (values.is_string()) {
      ...
      if (label == "corr") {
        if (giver.contains("w")) {
          add_exp_factor_(factor::BinaryFactorExponential::fromSparseDomainGen(
        ...
        if (giver.contains("w")) {
          auto values = misc::Intervals::fromGen(
      ...
      if (giver.contains("w")) {
        add_exp_factor_(factor::BinaryFactorExponential::fromSparseDomain<true>(
            {var_first_size, var_second_size}, std::move(combs)));
      } else {
        auto factor = factor::BinaryFactor::fromSparseDomain<true>(
            {var_first_size, var_second_size}, std::move(combs));
        model.add_binary_factor(std::move(factor), var_first_index,
                                var_second_index);
      }
