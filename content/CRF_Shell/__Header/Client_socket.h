...
public:
	Client_Socket(CRF_Shell* shell_to_link, const std::string& address, const std::string& port) : Client(shell_to_link), Connection(address , port) { this->Connection.InitConnection(); this->__allow_loop_to_start(); };
private:
