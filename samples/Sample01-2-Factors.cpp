...
int main() {
  SAMPLE_SECTION("Factors construction", "4.1.2.1", [] {
    ...
    auto Phi_AB = Factor<2>::fromSparseDomain<true>({4, 4}, std::move(domain));
  ...
  SAMPLE_SECTION("Find specific combination in Factors", "", [] {
    ...
    auto factor =
        Factor<4>::fromSparseDomain<true>({2, 2, 3, 3}, std::move(domain));
