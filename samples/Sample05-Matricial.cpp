...
int main() {
  SAMPLE_SECTION(
      ...
      "4.5", [] {
        ...
        if (matrix_size > 5) {
          threads =
              3; // use more threads to propagate the belief for big matrices
        }
        ...
        xml::Exporter::exportToFile(Matrix,
                                    xml::ExportInfo{"Matrix.xml", "Matrix"});
        // save the file into an xml (just as an example)
...
Graph makeMatrix(const std::size_t &matrix_size, const std::size_t &var_size,
  ...
  FactorExponential factor(
      Factor{VariablesSoup{A, B}, Factor::SimplyAntiCorrelatedTag{}},
      weight_correlation);
