...
class FactorCorrelationTest 
: public ::testing::Test
, public factor::modif::Factor { 
protected: 
    FactorCorrelationTest(bool corrOrAnti)
      : factor::modif::Factor(std::set<VariablePtr>{makeVariable(4, "A"), makeVariable(4 , "B"), makeVariable(4 , "C")}, corrOrAnti) {
    };
};
...
TEST_F(SimpleAntiCorrelationTest, simpleAntiCorrelation) {
    ...
    for(std::size_t v=0; v<varSize; ++v) {
        EXPECT_EQ(this->values->end(), this->values->find(Combination(vals.data(), vals.size())));
        for(std::size_t k=0; k<vals.size(); ++k) {
            ++vals[k];
        }
    }
...
TEST(ConstFactor, marginalization) {
    ...
    factor::modif::Factor distrABCD(std::set<categoric::VariablePtr>({ A, B, C, D }));
    ...
    {        
        iterator::forEach(rangeBD, [&vals, &distrABCD](const Range& rangeBD){
            vals = std::vector<std::size_t>{1, rangeBD.get().data()[0], 1, rangeBD.get().data()[1]};
...

    {
        ...
        vals = {0,1};
        factor::cnst::Factor distrBD(distrABCD, Combination(vals.data(), vals.size()), {A,C});
        rangeBD.reset();
...
TEST(ConstFactor, importFromFile) {
    factor::cnst::Factor factor(std::set<VariablePtr>{makeVariable(2, "A"), makeVariable(2, "B")}, std::string(TEST_FOLDER) + std::string("FactorDescription"));
}
