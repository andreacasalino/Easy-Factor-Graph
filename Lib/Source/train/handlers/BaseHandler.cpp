...
namespace EFG::train::handler {
    BaseHandler::BaseHandler(std::shared_ptr<distribution::factor::modif::FactorExponential> factor)
        : factor(factor) {
    }

    void BaseHandler::setTrainSet(TrainSetPtr newSet, const std::set<categoric::VariablePtr>& modelVariables) {
