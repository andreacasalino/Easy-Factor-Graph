...
namespace EFG::distribution::factor::cnst {
    ...
    Factor::Factor(const Distribution& o)
        : Factor(o.getGroup().getVariables()) {
    ...
    std::set<categoric::VariablePtr> mergeGroups(const std::set<const Distribution*>& distr) {
        if (distr.size() <= 1) {
    ...
    Factor::Factor(const Distribution& toMarginalize, const categoric::Combination& comb, const std::set<categoric::VariablePtr>& evidences)
        : Factor(getComplementary(toMarginalize.getGroup().getVariables(), evidences)) {
        ...
        std::for_each(evidences.begin(), evidences.end(), [&](const categoric::VariablePtr& v){
        ...
        auto extractRemainingComb = [&indexRemaining](const categoric::Combination& combThis){
    ...
    void fillCombination(const std::list<std::string>& slices, categoric::Combination& comb, float& image) {
    ...
    Factor::Factor(const std::set<categoric::VariablePtr>& group, const std::string& fileName)
        : Factor(group) {
