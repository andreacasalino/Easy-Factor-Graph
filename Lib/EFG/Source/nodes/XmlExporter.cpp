...
namespace EFG::nodes {
    ...
    void XmlExporter::exportToXml(const std::string& path, const std::string& name) const {
		...
		auto tunabClusters = tunabStructurePtr->getFactorsTunable();
		std::for_each(tunabClusters.begin(), tunabClusters.end(), [&exp_root](const std::vector<std::shared_ptr<distribution::factor::modif::FactorExponential>>& cluster) {
