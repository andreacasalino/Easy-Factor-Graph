...
namespace EFG {
	...
	private:
		void Reset() override {};
		float advance(Graph_Learnable* model_to_advance, const Training_set::Usable_Training_set& train_set, const bool& force_alfa_rec);
	// data
	...
	float I_Trainer::Likelihhod_Evaluator::operator()(const Training_set::Usable_Training_set& set){

		vector<const size_t*> temp;
		temp.reserve(set.size());
		auto it = set.get_iter();
		while (it.is_not_at_end()) {
			temp.push_back(*it);
			++it;
		}

		return this->Evaluator.Get_TrainingSet_Likelihood(temp);
	...
	void I_Trainer::Entire_Set::Train(Graph_Learnable& model_to_train, Training_set& training_set, const unsigned int& Max_Iterations , std::list<float>* descend_story) {
		...
		float variation;
		Training_set::Usable_Training_set Set(training_set, model_to_train);
		unique_ptr<I_Trainer::Likelihhod_Evaluator> lklhd_eval;
		if(descend_story != NULL) lklhd_eval.reset(new Likelihhod_Evaluator(model_to_train));
		for (unsigned int k = 0; k < Max_Iterations; ++k) {
			variation = this->adv_strtgy->advance(&model_to_train, Set, false);
			if (descend_story != NULL)  descend_story->push_back((*lklhd_eval)(Set));
	...
	void I_Trainer::Stoch_Set::Train(Graph_Learnable& model_to_train, Training_set& training_set, const unsigned int& Max_Iterations , std::list<float>* descend_story) {
		...
		this->adv_strtgy->Reset();
		float variation;
		Training_set::Usable_Training_set Set(training_set, model_to_train, this->Percentage);
		unique_ptr<I_Trainer::Likelihhod_Evaluator> lklhd_eval;
		if(descend_story != NULL) lklhd_eval.reset(new Likelihhod_Evaluator(model_to_train));
		for (unsigned int k = 0; k < Max_Iterations; ++k) {
			if(k > 0) Set.Resample();
	...
	float I_Trainer::Fixed_step::advance(Graph_Learnable* model_to_advance, const Training_set::Usable_Training_set& train_set, const bool& force_alfa_rec) {

		vector<float> grad_att = model_to_advance->Get_tunable_grad(train_set, force_alfa_rec);

		vector<float> w_att = model_to_advance->Get_tunable();
