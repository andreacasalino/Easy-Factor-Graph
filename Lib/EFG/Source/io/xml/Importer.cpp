...
namespace EFG::io::xml {
	...
	std::shared_ptr<distribution::factor::cnst::Factor> importFactor(const std::string& prefix, xmlPrs::Tag& tag, const std::set<categoric::VariablePtr>& variables) {
		...
		for (auto it = itDistr.begin(); it != itDistr.end(); ++it) {
			...
			factor->setImageRaw(categoric::Combination(comb.data(), comb.size()), static_cast<float>(std::atof(val->c_str())));
		}
		return factor;
	};
	...
	std::map<std::string, std::size_t> Importer::importComponents(const std::string& filePath, const std::string& fileName, const std::pair<strct::InsertCapable*, strct::InsertTunableCapable*>& components) {
