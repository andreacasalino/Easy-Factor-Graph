...
namespace EFG::strct {
    ...
    public:
         ...
         */
        std::vector<std::vector<std::shared_ptr<distribution::factor::modif::FactorExponential>>> getFactorsExp() const;

        /**
         * @return the weights of the tunable clusters. For each cluster only 1 value is returned, since it is shared among the elements in the same cluster.
         */
    ...
    protected:
         ...
         */
        std::map<std::shared_ptr<distribution::factor::modif::FactorExponential>, std::size_t> factorsExp;
