...
namespace EFG {
	...
	protected:
		class Likelihhod_Evaluator{
		public:
			Likelihhod_Evaluator(Graph_Learnable& model) : Evaluator(model.Get_Energy_Evaluator(model.Get_All_variables_in_model())) {};
			
			float operator()(const Training_set::Usable_Training_set& set);
		private:
			Node::Node_factory::Energy_Evaluator			Evaluator;
		};	
	private:
		...
		public:
			...
			virtual float advance(Graph_Learnable* model_to_advance, const Training_set::Usable_Training_set& train_set, const bool& force_alfa_rec) = 0; //return L_infinity norm of weigth variation
		};
