...
namespace EFG::train {
    template<typename TrainSetT = BasicTrainSet, typename LineSearcherT = YundaSearcher, typename HessianApproximatorT = BFGS>
    class QuasiNewton
    ...
    protected:
        inline void descend() override { 
            Vect dierction = this->invHessianApprox * this->getGradient();
            direction *= -1.f;
